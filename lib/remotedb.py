...
def motion(mac, start_date, end_date):
    # Define the SQL query with placeholders
    # uuid = getuuid(mac)
    # if uuid is None:
        # print(f"DB returend empty UUID value for {mac} during time period {start_date} to {end_date}")
        # return None
    sql = f"""SELECT
                epoch_time/1000 AS time,
                uuid,
                (('x' || REPLACE(LEFT(uuid, 8), '-', ''))::bit(32)::integer * 9.8 * 0.00390625) AS x_axis,
                (('x' || REPLACE(SUBSTRING(uuid FROM 10 FOR 8), '-', ''))::bit(32)::integer * 9.8 * 0.00390625) AS y_axis,
                (('x' || REPLACE(SUBSTRING(uuid FROM 19 FOR 8), '-', ''))::bit(32)::integer * 9.8 * 0.00390625) AS z_axis
            FROM
                btdata.anchorsdata
            WHERE
                mac = '{mac}'
            AND TO_TIMESTAMP(epoch_time/1000) >= TO_TIMESTAMP(%s, 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP(epoch_time/1000) <= TO_TIMESTAMP(%s, 'YYYY-MM-DD HH24:MI:SS');
    """
    return query_db(query=sql, start_date=start_date, end_date=end_date)
...
SELECT 
    ...
    (('x' || REPLACE(SUBSTRING(uuid FROM 10 FOR 8), '-', ''))::bit(32)::integer * 9.8 * 0.00390625) AS y_axis,
    (('x' || REPLACE(SUBSTRING(uuid FROM 19 FOR 8), '-', ''))::bit(32)::integer * 9.8 * 0.00390625) AS z_axis
FROM btdata.anchorsdata
WHERE epoch_time >= extract(epoch from now() - interval '90 minutes')
AND
mac = '00:8c:10:30:02:7a' 
;
